...
export default function AddLesson({ sectionId, onClose }: AddLessonProps) {
  ...
  const { mutate: createLesson, isPending } = useAppMutation({
    url: "/lessons/",
    type: "post",
    onSuccess: () => {
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-xl rounded-2xl shadow-xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-150">
        ...
        <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Add Lesson Media</h2>
            <p className="text-xs text-slate-400 mt-0.5 font-medium">Target Section: {sectionId}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          {({ errors, touched, values, setFieldValue }) => (
            <Form className="divide-y divide-slate-100">
                ...
                <div>
                  ...
                  <div className="prose max-w-none mt-1.5 Jodit-editor-custom-wrapper">
                    <JoditEditor
                      ...
                      onChange={() => {}} // Stabilizes typing response
                    />
                  </div>
                    ...
                    <div className="mt-1.5 border border-slate-200 bg-slate-50 rounded-xl p-3 flex flex-col gap-3">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2 text-sm font-semibold text-emerald-700">
                          <PlayCircle size={16} />
                          Resource Asset Cached Successfully
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                      ...
                      <video src={videoUrl} controls className="w-full rounded-lg bg-black shadow-inner max-h-[180px]" />
                    </div>
                  )}
              ...
              <div className="px-6 py-4 bg-slate-50 flex flex-row-reverse gap-3">
                <button
                  type="submit"
                  disabled={isPending || uploading || !videoUrl}
                  className="w-full sm:w-auto min-w-30 inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 disabled:bg-slate-200 disabled:text-slate-400 text-white text-sm font-medium px-5 py-2.5 rounded-xl shadow-sm transition-colors duration-150"
                >
                  {isPending ? (
                    <>
